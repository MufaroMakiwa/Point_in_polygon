...
#
# HexCodes are short, 2-6 character codes that can be used instead
...
CODE_ALPHABET_ = '2345679ABCDEFGHJKLMNPQRSTUVWYZ'
...
PAIR_RESOLUTIONS_ = [30.0, 1.0, 1/30, 1/900, 1/27000]
...
GRID_SIZE_DEGREES_ = 1/27000
"""
 Coordinates of a decoded HexCode.
...
To be valid, all characters must be from the HexCode character
...
A short HexCode is a sequence created by removing four or more
digits from an HexCode. It must include a separator
character.
"""
def isShort(code):
    # Check it's valid.
    ...
    return False
"""
 Determines if a code is a valid full HexCode.
 Not all possible combinations of HexCode characters decode to
...
def computeLatitudePrecision(codeLength):
    if codeLength <= 10:
        return pow(30, math.floor((codeLength / -2) + 2))
    return pow(30, -3) / pow(GRID_ROWS_, codeLength - 10)
...
def normalizeLongitude(longitude):
    ...
    return longitude
"""
 Encode a location into an HexCode.
...
def encode(latitude, longitude, codeLength=PAIR_CODE_LENGTH_):
    if codeLength < 2 or (codeLength < PAIR_CODE_LENGTH_ and
                          codeLength % 2 == 1):
        raise ValueError('Invalid HexCode length - ' +
                         str(codeLength))
...
# print(encode(51.521089, -0.203542, codeLength=PAIR_CODE_LENGTH_))
"""
 Decodes an HexCode into the location coordinates.
 Returns a CodeArea object that includes the coordinates of the bounding
 box - the lower left, center and upper right.
 Args:
   code: The HexCode to decode.
 Returns:
...
def decode(code):
    if not isFull(code):
        raise ValueError(
            'Passed HexCode is not a valid full code - ' + str(code))
...
"""
 ...
 Given a short HexCode of between four and seven characters,
 ...
 Args:
   code: A valid HexCode character sequence.
 ...
 Returns:
   The nearest full HexCode to the reference location that matches
...
"""
 Remove characters from the start of an HexCode code.
 This uses a reference location to determine how many initial characters
 can be removed from the HexCode code. The number of characters that can be
